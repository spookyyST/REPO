    ...
    <div class="container">
        <div class="catalog-tabs" aria-label="Категории каталога">
            ...
            <?php foreach ($catalog['categories'] as $category) : ?>
                <?php $category_url = add_query_arg('product_category', $category->slug, $catalog_url); ?>
                ...
                <form class="catalog-filter-form" method="get" action="<?php echo esc_url($catalog_url); ?>">
                    ...
                    <?php if ($catalog['filter_groups']) : ?>
                        <?php foreach ($catalog['filter_groups'] as $group) : ?>
                            <?php $active_values = isset($catalog['active_filters'][$group['key']]) ? $catalog['active_filters'][$group['key']] : array(); ?>
            ...
            <div class="product-area">
                <?php if ($catalog['products']) : ?>
                    ...
                    <?php if ($catalog['total_pages'] > 1) : ?>
                        ...
                        $page_numbers = array_values(array_unique(array_filter(array(1, $current_page - 1, $current_page, $current_page + 1, $total_pages), function ($page_number) use ($total_pages) {
